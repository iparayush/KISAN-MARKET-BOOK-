...
import { Language, AdminAlert, MandiCenter, AuditLogEntry } from '../types';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  return (
    <div className="flex flex-col w-full space-y-4 max-w-5xl mx-auto pb-12 animate-fade-in">
      ...
      {/* System Operational Audit Trail (SIH Section 48) */}
      <div className="bg-surface-container-lowest rounded-2xl p-4 border border-outline-variant/30 shadow-sm flex flex-col space-y-3">
        <div className="flex items-center justify-between border-b border-outline-variant/30 pb-2">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-primary text-[20px]">history_edu</span>
            <h3 className="text-xs font-bold text-on-surface uppercase tracking-wider">
              {language === 'mr' ? 'प्रणाली ऑपरेटर व व्यवहार ऑडिट नोंद वही (Audit Log)' : 'System Operational Audit Log'}
            </h3>
          </div>
          <span className="text-[11px] text-on-surface-variant font-mono font-semibold">
            {language === 'mr' ? 'अपरिवर्तनीय नोंद (Cryptographic Log)' : 'Immutable System Records'}
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-surface-container-low text-on-surface-variant border-b border-outline-variant/30">
                <th className="p-2.5 font-bold">{language === 'mr' ? 'आयडी / वेळ' : 'Log ID / Time'}</th>
                <th className="p-2.5 font-bold">{language === 'mr' ? 'ऑपरेटर / वापरकर्ता' : 'Operator / User'}</th>
                <th className="p-2.5 font-bold">{language === 'mr' ? 'केलेली कृती' : 'Action Performed'}</th>
                <th className="p-2.5 font-bold">{language === 'mr' ? 'केंद्राचे नाव' : 'Mandi Centre'}</th>
                <th className="p-2.5 font-bold">{language === 'mr' ? 'तपशील' : 'Metadata / Details'}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/20">
              {(auditLogs || INITIAL_AUDIT_LOGS).map((log) => (
                <tr key={log.id} className="hover:bg-surface-container-low/50 transition-colors">
                  ...
                  <td className="p-2.5 font-medium">
                    <span className="block text-on-surface font-bold">{log.userName}</span>
                    <span className="text-[10px] text-on-surface-variant font-mono">ID: {log.userId}</span>
                  </td>
                  <td className="p-2.5">
                    <span className="px-2 py-0.5 rounded bg-primary-container/40 text-on-primary-container font-semibold text-[11px] inline-block">
                      {language === 'mr' ? log.actionMr : log.actionEn}
                    </span>
                  </td>
                  <td className="p-2.5 text-on-surface font-medium">{log.centreName}</td>
                  <td className="p-2.5 text-[11px] text-on-surface-variant font-mono">{log.details}</td>
                </tr>
              ))}
            </tbody>
          </table>
